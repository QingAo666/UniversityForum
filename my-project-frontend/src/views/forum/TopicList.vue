...
const today = computed(() => {
  ...
  const month = currentDate.getMonth() + 1; // 注意getMonth()是从0开始计数的，所以要加1
...
<template>
  <div style="display: flex; margin: 20px auto;gap: 50px; max-width: 1100px">
    <div style="flex: 1">
      <light-card>
        <div class="create-topic" @click="editor = true">
          <el-icon><EditPen/></el-icon> 点击发表主题...
        </div>
        <div style="margin-top: 10px;display: flex; gap: 13px; font-size: 17px; color: grey">
            ...
            <light-card v-for="item in topics.list" class="topic-card" @click="router.push('/index/topic-detail/'+item.id)">
              <div style="display: flex">
                ...
                <div style="margin-left: 7px; transform: translateY(-2px)" >
                  <div style="font-size: 14px; font-weight: bold;margin-bottom: 2px;transform: translateY(-1px)">
                    {{ item.username }}</div>
              ...
              <div style="display:flex;gap:20px;font-size: 14px;margin-top: 15px; opacity: 0.8">
                <div>
      ...
      <div style="position: sticky; top: 20px">
        ...
        <light-card style="margin-top: 10px">
          ...
          <div style="font-size: 14px;margin: 10px;color: grey">
            大家好！ 近期校园论坛将进行系统升级和维护，为了提升用户体验和网站性能，我们计划进行更新并可能暂时关闭部分功能。<br/>
            感谢各位用户的理解和支持！如有任何问题或建议，请随时联系我们。
          </div>
        </light-card>
        <light-card style="margin-top: 10px">
          <div style="font-weight: bold">
